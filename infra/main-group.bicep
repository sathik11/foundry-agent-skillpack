// MAINTAINER/CI-ONLY — RG-scoped sibling of main.bicep (W5-T3).
// Use when the dedicated RG is PRE-CREATED so the CI service principal needs only
// RG-scoped roles (no subscription-scope grant). Deploy with:
//   az deployment group create -g <rg> --template-file main-group.bicep ...
// This is a transform of main.bicep (subscription-scoped): identical params/modules/outputs,
// minus the RG-creation resource and the explicit `scope: rg` on each module. Keep in sync
// with main.bicep when the vendored template changes.
