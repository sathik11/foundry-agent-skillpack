...
@description('Provision a Cosmos DB account in the resource group (used for capability host or general agent thread storage).')
...
// ── Capability host BYO resource provisioning ──────────────────────────────
// When enableCapabilityHost=true, provision dedicated Cosmos/Storage/Search if
// they are not already in additionalDependentResources, then create the
// project-level capability host that wires them together.
// Provisioning is independent of capability host enablement so the resources
// exist in the RG even when v2 hosted agents manage thread storage themselves.
var shouldCreateCapCosmos = enableCosmos && empty(cosmosConnectionNameOverride)
var shouldCreateCapHostStorage = enableStorage && !hasStorageConnection
var shouldCreateCapHostSearch = enableSearch && !hasSearchConnection
// Resolved capability-host connection name strings (passed by name to the capHost resource)
var capCosmosConnectionName = empty(cosmosConnectionNameOverride) ? 'cap-cosmos-${resourceToken}' : cosmosConnectionNameOverride
var capStorageConnectionName = hasStorageConnection ? storageConnectionName : 'cap-storage-${resourceToken}'
var capSearchConnectionName = hasSearchConnection ? searchConnectionName : 'cap-search-${resourceToken}'
...
// ── Capability host BYO resources ──────────────────────────────────────────

// Cosmos DB for capability host thread storage (always created when enableCapabilityHost=true
// unless an existing cosmos connection name is supplied via cosmosConnectionNameOverride)
module capHostCosmos '../database/cosmos.bicep' = if (shouldCreateCapCosmos) {
  name: 'cap-host-cosmos'
  params: {
    location: location
    tags: tags
    resourceName: 'cosmos-cap-${resourceToken}'
    connectionName: capCosmosConnectionName
    principalId: principalId
    principalType: principalType
    aiServicesAccountName: aiAccount.name
    aiProjectName: aiAccount::project.name
...
// Dedicated blob storage for capability host (only when storage not already in additionalDependentResources)
module capHostStorage '../storage/storage.bicep' = if (shouldCreateCapHostStorage) {
  name: 'cap-host-storage'
  params: {
    location: location
    tags: tags
    resourceName: 'stcap${resourceToken}'
    connectionName: capStorageConnectionName
    principalId: principalId
    principalType: principalType
    aiServicesAccountName: aiAccount.name
    aiProjectName: aiAccount::project.name
...
// Dedicated AI Search for capability host vector store (only when search not already in additionalDependentResources)
module capHostSearch '../search/azure_ai_search.bicep' = if (shouldCreateCapHostSearch) {
  name: 'cap-host-search'
  params: {
    location: location
    tags: tags
    resourceName: 'search-cap-${resourceToken}'
    connectionName: capSearchConnectionName
    // storage is required by the search module for its knowledge-base container
    storageAccountResourceId: shouldCreateCapHostStorage
      ? capHostStorage!.outputs.storageAccountId
      : (hasStorageConnection ? storage!.outputs.storageAccountId : '')
    containerName: 'agent-knowledge'
    principalId: principalId
    principalType: principalType
    aiServicesAccountName: aiAccount.name
    aiProjectName: aiAccount::project.name
...
// Project-level capability host — BYO thread/vector/blob storage
// Ordering rule: account-level capability host MUST be created first (dependsOn aiFoundryAccountCapabilityHost)
resource projectCapabilityHost 'Microsoft.CognitiveServices/accounts/projects/capabilityHosts@2025-10-01-preview' = if (enableHostedAgents && enableCapabilityHost) {
  name: '${aiAccount.name}/${aiAccount::project.name}/agents'
  properties: {
    capabilityHostKind: 'Agents'
    threadStorageConnections: [capCosmosConnectionName]
    vectorStoreConnections: [capSearchConnectionName]
    storageConnections: [capStorageConnectionName]
  }
  dependsOn: [
...
output dependentResources object = {
  ...
  cosmos: {
    accountName: shouldCreateCapCosmos ? capHostCosmos!.outputs.cosmosAccountName : ''
    connectionName: enableCapabilityHost ? capCosmosConnectionName : ''
    endpoint: shouldCreateCapCosmos ? capHostCosmos!.outputs.cosmosDocumentEndpoint : ''
  }
  capHostStorage: {
    accountName: shouldCreateCapHostStorage ? capHostStorage!.outputs.storageAccountName : ''
    connectionName: enableCapabilityHost ? capStorageConnectionName : ''
  }
  capHostSearch: {
    serviceName: shouldCreateCapHostSearch ? capHostSearch!.outputs.searchServiceName : ''
    connectionName: enableCapabilityHost ? capSearchConnectionName : ''
  }
}
