...
// Capability host bootstrap requires Storage Blob Data **Owner** (not Contributor)
// so the project MI can write container ACLs while creating BYO blob containers.
resource storageRoleAssignment 'Microsoft.Authorization/roleAssignments@2022-04-01' = if (!empty(aiServicesAccountName) && !empty(aiProjectName)) {
  name: guid(storageAccount.id, aiAccount.id, 'ai-storage-blob-owner')
