...
- [x] **TD-29 opened (adopt + integrate, v0.24 candidate).** AGT as a declarable `runtime_governance: agt` layer in `agent-capabilities.yaml` — container `requirements.txt` injection, agent template `govern(...)` wraps, OTel cross-link to AGT decisions, `/audit-drift` reconciliation of declared policy file. `foundry-guardrails` skill gains a Layer 0 (deterministic runtime enforcement) ahead of the existing four-layer model. Deferred from v0.23.0 because AGT is v3.7.0 "Public Preview" with breaking-change risk; want a stable AGT minor as the integration target.
