...
10. **SDK pins + api-versions have ONE source of truth: `maintenance/versions.yaml`.** Never hand-edit a pinned dependency inside `templates/requirements.txt.template`, `templates/langgraph-byo/requirements.txt.template`, `sdk-surface.md` "Pinned versions", the README "Supported SDK versions" block, or `maintenance/SUPPORTED.md` — those regions live between `versions:auto:*` markers and are **generated**. Edit `maintenance/versions.yaml`, then run `python maintenance/scripts/sync-versions.py --write`. CI (`versions-guard.yml` → `sync-versions.py --check`) fails any PR where a managed block drifts. Bumps are *findings* (`maintenance/findings-backlog.md`) that must pass the E2E harness + approval gate before merge — never blind-bump. (Maintainer automation lives at repo root under `maintenance/` + `infra/` + `.github/workflows/`, NEVER under `.apm/`, so it never ships via `apm install`.)
