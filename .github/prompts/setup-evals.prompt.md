...
Load `${input:agent_path}/agent-capabilities.yaml`. The wrapper derives evaluators from `evals.role` + capabilities (see [evaluator-catalog.md](../../apm_modules/_local/foundry-agent-skillpack/.apm/skills/foundry-evals/evaluator-catalog.md)). Inputs override the manifest if both present.
...
The wrapper hard-fails preflight with the supported region list if the project's region is unsupported. In that case, fall back to PyRIT-in-CI — see [foundry-guardrails/redteam-evals.md](../../apm_modules/_local/foundry-agent-skillpack/.apm/skills/foundry-guardrails/redteam-evals.md).
...
- Quality score drops may indicate model drift; cross-reference with [foundry-observability](../../apm_modules/_local/foundry-agent-skillpack/.apm/skills/foundry-observability/SKILL.md) KQL.
